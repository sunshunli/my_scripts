...
$.newShareCodes = ['06117194f46743c5029c4258c53990dc', '5eb09cdf44c39e29347bdd0e5bb77b18', 'ff42f4f24c905d9695f90097955fb4bb', '575387d46823a35da8878efebdb7e63c']
!(async () => {
  if (!cookiesArr[0]) {
    ...
    $.get(taskUrl('jdh_task_queryTask', body), async (err, resp, data) => {
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data)
            if (data.data && data.data.length > 0) await doTask(taskType, data.data[0].id, infoId)
...
function rewardTask(taskType, taskId, infoId) {
  // 会报 no access 无解
  let body = { channel: 'jdhapp', m_patch_appid: 'jdh', taskId: taskId, taskType: taskType, infoId: infoId }
  return new Promise((resolve) => {
